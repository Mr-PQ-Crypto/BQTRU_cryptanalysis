...
import json
from BQTRU import BQTRUGenerator
from utils import (get_key_norm, get_norm, is_it_zero, is_it_ternary, is_it_pm_2, add_vectors_with_centerlifting,
                   ...
                   dump_seed, dump_blocksize_for_message_attack, scalar_multiply, addCN, substractCN, is_it_boundedby)
...
class Attack:
    """
    Attack on BQTRU that breaks the moderate parameter set claimed to achieve 2^{92} message security and 2^{166} key
    security.

    To attack the moderate parameter sets, one can run:
    - For key attack:
            python3 attack.py 196 'reduction' -q=113 --verbose=True --dump=True --bkz_betas=40:75 --attack_type=0 --option=1 --trials=100
    - For message attack:
            python3 attack.py 196 'reduction' -q=113 --verbose=True --dump=True --bkz_betas=53:75 --attack_type=1 --option=1 --trials=100

    To attack a toy example:
    - For key attack:
             python3 attack.py 100 'reduction'  --verbose=True --dump=True --bkz_betas=3:75 --attack_type=0 --option=1 --trials=100
    - For message attack:
             python3 attack.py 100 'reduction'  --verbose=True --dump=True --bkz_betas=3:75 --attack_type=1 --option=1 --trials=100
    ...
    def __init__(self, params):
        ...
        self.generator = BQTRUGenerator(self.n, self.q, self.seed, self.h, self.option, self.empty_fset, self.weak_instance, self.guess, self.attack_type)
